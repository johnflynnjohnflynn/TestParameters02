...
TestParameters02AudioProcessor::TestParameters02AudioProcessor()
    ...
    : stateAB {*this},
      ...
      gainParam         {new parameter::ParamStepListenGain {"gainID",     "Gain",           -12.0f,    12.0f,   0.0f,   24,   0.0f, *gainStepSizeParam}},
      ...
      gain2Param        {new parameter::ParamStep           {"gain2ID",    "Gain2",          -12.0f,    12.0f,   0.0f,   24,   0.0f}},
...
void TestParameters02AudioProcessor::processBlock (AudioSampleBuffer& buffer, MidiBuffer& /*midiMessages*/)
{
    ...
    band1.process(buffer, totalNumInputChannels, *gainParam, freqNorm, *qParam);
}
